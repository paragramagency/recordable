...
<!-- Borrowed from MyST "roles": {action}`args`. Name in braces, args in a backtick
     span (so they get native blue inline-code highlighting). No parser dep — the
     {name} prefix + backtick span is matched by our own regex. -->
...
Here are the results. {scroll}`#results` Every grade comes with a rationale you can audit, {zoom}`1.5 #rationale duration=800` so nothing is a black box. {wait}`2000`
